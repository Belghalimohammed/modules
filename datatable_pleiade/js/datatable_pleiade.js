    ...
    attach: function (context, settings) {
      setTimeout(function () {
        once("DatatableDocBehavior", "body", context).forEach(function () {
          ...
          // --- MODIFICATION START ---
          // Use event delegation for the click handler because the button is dynamically created.
          // We attach the listener to a parent ('#document_recent_id') that exists on page load.
          // The handler will execute when a click event bubbles up from an element with the id 'reloadDatatable'.
          $('#document_recent_id', context).on('click', '#reloadDatatable', function() {
            // Before reloading, destroy the existing DataTable to prevent an error.
            if ($.fn.DataTable.isDataTable('#tablealldocs')) {
              $('#tablealldocs').DataTable().destroy();
            }
            // Optional: clear the table body to prevent seeing old data while new data loads.
            $('#tablealldocs tbody').empty();
            ...
            xhr.onload = function () {
              if (xhr.status === 200) {
                ...
                if (donnees && donnees != '0') {
                  ...
                  for (var i = 0; i < donnees.length; i++) {
                    if (
                      ...
                      var type = "Notification"; // Assuming 'Nextcloud' is the type for this item
				...
				<td>" +
                      ...
                      var objectDate = donnees[i].creation;
                      var etat =
                        '<span class="badge py-2 px-4 bg-danger">' +
                        donnees[i].status +
                        "</span>";
                      var lien_detail =
                        '<a target="_blank" href="' +
                        donnees[i].fileUrl +
                        '"><i class="fa fa-2x fa-signature" aria-hidden="true"></i></a>';
                      var document_row =
                        "\
                              <tr>\
				<td>" +
